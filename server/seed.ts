...
import { STATIC_PRODUCTS } from "../client/src/lib/staticData";
...
async function seed() {
  ...
  // ─── Products & Images ────────────────────────────────────────────────
  console.log("📦 Seeding 18 official products from STATIC_PRODUCTS...");
  for (const sp of STATIC_PRODUCTS) {
    const inserted = await db
      ...
      .values({
        ...
        isNew: sp.isNew ?? false,
        warrantyMonths: sp.warrantyMonths ?? 12,
        specifications: sp.specifications,
        tags: JSON.stringify(["official", sp.category.toLowerCase()]),
      })
      .returning({ id: products.id });

    const prodId = inserted[0]?.id;
    if (prodId && sp.imageUrl) {
